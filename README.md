...
```
00ny.github.io/
├── index.html                    ← 랜딩: 목차 → 캐러셀 → 이력서 → 카드 3영역 → 외부 링크
├── assets/
...
프로젝트 순서(카드·이전/다음 네비)는 **창작 2 → 역기획 2 → 제안 1**. 헤스티는 엔드필드 분석에서 파생된 제안이라 엔드필드 뒤에 온다.
...
- **index 전용** — 목차 바, 3D 캐러셀 히어로, 카드, 태그칩, 외부 링크, 이력서
...
- 카드 제목·태그라인·칩은 마크업의 `data-title` / `data-tagline` / `data-chip`이 원본이다.
...
| 하고 싶은 것 | 고칠 곳 |
|---|---|
| 이력서 내용 채우기 | `index.html`의 `#resume` — `{{...}}` 플레이스홀더 |
| 캐러셀 문구 교체 | `index.html`의 `.cr-card`의 `data-title` / `data-tagline` / `data-chip` |
| 캐러셀 카드 순서·시작 카드 | 마크업의 `.cr-card` 순서 (첫 번째가 시작 카드) |
