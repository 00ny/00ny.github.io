...
| **리뷰 추가**(스토브 글이 늘 때) | `index.html`의 `#analysis` — `.rvw-grid` 안에 `.rvw` 카드 하나를 통째로 복사해 href·문구만 교체(2열 그리드가 자동 개행) |
| **유튜브 영상 추가·교체** | `index.html` 하단 `<script>`의 `YT_VIDEOS` 배열 — `{id, title}` 한 줄 추가/수정(마크업 불필요, 카드 자동 생성) |
| 유튜브 **실시간 조회수 켜기** | 같은 스크립트의 `YT_API_KEY`에 YouTube Data API v3 키를 넣으면 각 영상 조회수가 자동 표시(비우면 조회수 줄 생략). 키는 도메인 제한 권장 |
