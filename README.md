...
```
00ny.github.io/
├── index.html                    ← 랜딩: 목차 → 캐러셀 → 이력서(벤토) → 분석·리뷰 → 카드 3영역 → 외부 링크
├── assets/
...
| 리뷰 글·유튜브 영상 교체 | `index.html`의 `#analysis` — 리뷰는 `.rvw` 카드의 href·문구, 영상은 `.ytb-frame iframe`의 `src` (`youtube-nocookie.com/embed/<영상ID>`) |
